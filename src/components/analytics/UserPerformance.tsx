...
const UserPerformance: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const filteredTransactions = transactions.filter(transaction => {
      ...
      return txTime >= startTimestamp && txTime <= endTimestamp && transaction.status === "completed";
    });
  ...
  return <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <select value={activeReport} onChange={e => setActiveReport(e.target.value as any)} className="border rounded px-2 py-1 text-sm">
          ...
          <ReportExport data={userSalesData} filename="user-performance-report" />
        </div>
      </div>
          ...
          <Card className="mt-6">
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {userSalesData.length > 0 ? userSalesData.filter(user => user.transactions > 0).sort((a, b) => b.sales - a.sales).map(user => <TableRow key={user.userId}>
                          <TableCell>
      ...
      {activeReport === "comparison" && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <div className="h-[400px]">
                {userSalesData.filter(user => user.sales > 0).length > 0 ? <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={userSalesData.filter(user => user.sales > 0)} cx="50%" cy="50%" labelLine={false} outerRadius={150} fill="#8884d8" dataKey="sales" nameKey="name" label={({
                  name,
                  percent
                }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                        {userSalesData.filter(user => user.sales > 0).map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                      </Pie>
          ...
          <Card>
            ...
            <CardContent>
              <div className="h-[400px]">
                {userSalesData.filter(user => user.transactions > 0).length > 0 ? <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={userSalesData.filter(user => user.transactions > 0).sort((a, b) => b.transactions - a.transactions)} margin={{
                top: 5,
                right: 30,
                left: 20,
                bottom: 5
      ...
      {activeReport === "daily" && <Card>
          ...
          <CardContent>
            <div className="h-[500px]">
              {getDailySalesChartData().length > 0 ? <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={getDailySalesChartData()} margin={{
              top: 5,
              right: 30,
              left: 20,
              bottom: 5
                    ...
                    <Legend />
                    {userSalesData.filter(user => user.transactions > 0).map((user, index) => <Line key={user.userId} type="monotone" dataKey={user.name} stroke={COLORS[index % COLORS.length]} activeDot={{
                r: 8
              }} />)}
                  </LineChart>
                </ResponsiveContainer> : <div className="h-full flex items-center justify-center">
