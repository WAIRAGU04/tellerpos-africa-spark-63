    ...
    if (item.imageUrl) {
      ...
      return (
        <div className={`w-10 h-10 ${colorMap[item.color]} rounded-md flex items-center justify-center`}>
          {item.type === 'product' ? (
            <Package2 className="w-5 h-5 text-white opacity-50" />
          ) : (
            <FileText className="w-5 h-5 text-white opacity-50" />
          )}
        </div>
      );
    }
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3 flex-grow overflow-hidden">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(item.price)} each
                    </p>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button 
                      ...
                      className="text-red-500 hover:bg-red-100 dark:hover:bg-red-900/30 h-7 w-7"
                    >
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 p-4 mt-auto">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="font-medium">
            {new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(cartTotal)}
          </span>
        </div>
