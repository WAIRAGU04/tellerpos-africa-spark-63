...
const POSCheckout: React.FC<POSCheckoutProps> = ({ 
  ...
  const [splitAmounts, setSplitAmounts] = useState<Array<{method: PaymentMethod; amount: number}>>([]);
    ...
    if (paymentType === 'split') {
      ...
      if (Math.abs(totalSplit - cartTotal) > 0.01) {
        toast({
          ...
          description: `Split payments must equal the total amount of ${new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(cartTotal)}`,
          variant: "destructive",
        });
        return;
      }
      ...
      <div className="p-4 overflow-y-auto flex-1">
        ...
        {paymentType === 'full' ? (
          ...
          <POSSplitPayment 
            ...
            showCustomerSelect={() => setShowCustomerSelect(true)}
          />
        )}
      </div>
            ...
            <AlertDialogDescription>
              {paymentMethod === 'cash' ? (
                <>
                  ...
                  <span className="font-medium">{new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(parseFloat(amountTendered))}</span>
                  {' '}as cash payment.
                </>
              ) : paymentMethod === 'mpesa-till' ? (
                <>Please confirm you have received <span className="font-medium">{new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(cartTotal)}</span> via M-PESA Till.</>
              ) : paymentMethod === 'pochi-la-biashara' ? (
                <>Please confirm you have received <span className="font-medium">{new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(cartTotal)}</span> via Pochi La Biashara.</>
              ) : paymentMethod === 'bank-transfer' ? (
                <>Please confirm you have received <span className="font-medium">{new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(cartTotal)}</span> via Bank Transfer.</>
              ) : (
                <>Please confirm you have received <span className="font-medium">{new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(cartTotal)}</span> via the selected payment method.</>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
