...
const POSInventoryView: React.FC<POSInventoryViewProps> = ({ inventory, addToCart }) => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'products') {
      ...
      filtered = filtered.filter(item => 
        (item.type === 'product' && (item as Product).quantity > 0) ||
  ...
  const renderItemVisual = (item: InventoryItem) => {
    if (item.imageUrl) {
      ...
      return (
        <div className={`w-full h-24 ${colorMap[item.color]} rounded-t-md flex items-center justify-center`}>
          {item.type === 'product' ? (
            <Package2 className="w-12 h-12 text-white opacity-50" />
          ) : (
    ...
    // Fallback
    return (
      <div className="w-full h-24 bg-gray-200 dark:bg-tellerpos-dark-accent rounded-t-md flex items-center justify-center">
        {item.type === 'product' ? (
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground truncate">{
                          item.type === 'product' ? `Stock: ${(item as Product).quantity}` : 
                            (item as Service).isAvailable ? 'Available' : 'Unavailable'
                        }</p>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        {canAddToCart(item) ? (
                          ...
                          <Badge variant="secondary" className="bg-gray-200 dark:bg-gray-700 text-xs">
